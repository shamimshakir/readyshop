...
if(isset($_REQUEST['txtpd_id']) && $_REQUEST['txtpd_id'] !="0"){
    ...
    }else{$cond=" WHERE tbl_order_item.pd_id='".$_REQUEST['txtpd_id']."'";}
}
...
if(isset($_REQUEST['txtfromrec_date']) && $_REQUEST['txttorec_date'] !=NULL){
    ...
    $subhead="Order Date From ".$txtfromrec_date." To ".$txttorec_date;
    if($cond!=NULL){
        $cond.=" AND (tbl_order.od_date>='".$txtfromrec_date."' AND tbl_order.od_date<='".$txttorec_date."')";
    }else{$cond=" WHERE (tbl_order.od_date>='".$txtfromrec_date."' AND tbl_order.od_date<='".$txttorec_date."')";}
}
