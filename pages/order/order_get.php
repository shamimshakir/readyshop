...
$where .= " and `tbl_order`.`od_status` not in (5,6)";
$columns = array(
  0 => 'od_id',
  1 => 'od_no',
  2 => 'cl_id',
...
$sql = "SELECT
        tbl_order.od_id,
        tbl_order.od_no,
        tbl_customer.firstname as cl_id,
...
while ( $row = mysqli_fetch_row( $queryRecords ) ) {
 ...
 array_push( $edit, "<button class='dropdown-item' onclick='order_status_form({$row[0]})'  href='#'>Change Status</button>" );

 if ( $row[ 3 ] == "Due" ) {
   array_push( $edit, "<button class='dropdown-item' onclick='payment_amount_form({$row[0]})' href='#'>Received Payment</button>" );
 }

 if($row[ 7 ] == 1 && $row[ 3 ] == "Due"){
   array_push( $edit, "<button class='dropdown-item' onclick='order_items({$row[0]})' href='#'>Change Order Quantity</button>" );
 }

 array_push( $edit, "<button class='dropdown-item' onclick='order_details({$row[0]})'' href='#'>Order Details</button>" );
array_push( $edit, "</div> </div>" );


  // 	}

  array_push( $row_array, join( '', $edit ) );
  $data[] = $row_array;
}
// print_r($data);
$json_data = array(
